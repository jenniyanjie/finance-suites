...
import { TradingCostInput, TradingCostResult, TradingCostBreakdown, ApiResponse } from '../types';
 ...
 * @returns 费用明细
 */
function calculateSideFees(amount: number, rates: {
 ...
 * @returns 计算结果
 */
export const calculateTradingCost = (input: TradingCostInput): ApiResponse<TradingCostResult> => {
  try {
    // 输入验证
    ...
    const result: TradingCostResult = {
      buyBreakdown,
      sellBreakdown,
 ...
 * @returns 默认费率配置
 */
export const getDefaultTradingRates = (): Omit<TradingCostInput, 'buyPrice' | 'sellPrice' | 'shares'> => {
  return {
