...
import { DCAInput, DCAResult, DCAPeriodDetail, ApiResponse } from '../types';
...
const gaussianRandom = (rand: () => number): number => {
  // Box-Muller 变换
 ...
 * 计算DCA定投模拟
 */
export const calculateDCA = (input: DCAInput): ApiResponse<DCAResult> => {
  try {
    ...
    let totalShares = 0;
    let totalInvested = 0;
    const periodDetails: DCAPeriodDetail[] = [];
