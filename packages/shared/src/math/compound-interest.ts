 ...
 * - 复利：FV = P * (1 + r) ^ n
 */
export const calculateFutureValue = (input: FVInput): ApiResponse<FVResult> => {
  try {
    // 输入验证
 ...
 * 反向计算：给定终值和其他参数，计算本金
 */
export const calculatePrincipal = (FV: number, r: number, n: number, mode: 'simple' | 'compound'): ApiResponse<{ P: number }> => {
  try {
 ...
 */
export const calculateInterestRate = (FV: number, P: number, n: number, mode: 'simple' | 'compound'): ApiResponse<{ r: number }> => {
  try {
    validateNumber(FV, '终值', { min: 0.01 });
