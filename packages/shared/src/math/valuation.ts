 ...
 * PE估值计算
 */
export const calculatePEValuation = (input: PEInput): ApiResponse<PEResult> => {
  try {
 ...
 * DCF两阶段折现现金流估值
 */
export const calculateDCF = (input: DCFInput): ApiResponse<DCFResult> => {
  try {
    ...
    if (input.terminalValue.type === 'gordon') {
      ...
      if (gPerpetual >= input.r) {
        throw new ValidationError(
          '永续增长率',
          `永续增长率 (${gPerpetual}%) 必须小于折现率 (${input.r}%)`
        );
      }
    } else {
    ...
    // 第一阶段：逐年现金流折现
    const cashFlowDetails: DCFResult['cashFlowDetails'] = [];
    let pvSum = 0;
    ...
    let terminalValueAmount: number;
