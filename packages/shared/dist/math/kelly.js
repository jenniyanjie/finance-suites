 ...
 * 计算Kelly最优仓位
 */
export const calculateKellyPosition = (input) => {
    try {
        ...
        // 负Kelly表示负期望值，不应入场
        if (kellyRaw <= 0) {
            throw new ValidationError('期望值', `当前参数的期望值为负（Kelly=${(kellyRaw * 100).toFixed(2)}%），系统无正期望，建议不入场`);
        }
 ...
 */
export const calculateExpectancy = (p, b, riskAmount) => {
    try {
